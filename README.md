...
TUI 默认使用英语。需要简体中文 UI 和 assistant 回复时，可以使用 `-l zh` 或 `-l cn`。
...
- `-l en` / `--language en`：英语 UI 和 assistant 偏好。这是默认值。
- `-l zh` / `-l cn` / `--language zh`：简体中文 UI 和 assistant 偏好，专业术语会在更清晰时保持英文。
