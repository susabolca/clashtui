...
The TUI defaults to English. Use `-l zh` or `-l cn` for Simplified Chinese UI
...
- `-l en` / `--language en`: English UI and assistant preference. This is the
  default.
- `-l zh` / `-l cn` / `--language zh`: Simplified Chinese UI and assistant
  preference, with technical terms kept in English where that is clearer.
