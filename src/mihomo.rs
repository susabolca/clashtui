...
impl MihomoClient {
    ...
    pub async fn proxy_provider_proxy_names(&self, provider: &str) -> Result<Option<Vec<String>>> {
        let value: Value = self
            .send(self.client.get(self.url("/providers/proxies")))
            .await?;
        proxy_provider_proxy_names_from_value(&value, provider)
    }
...
fn proxy_provider_proxy_names_from_value(
    value: &Value,
    provider: &str,
    ...
    let proxies = provider
        ...
        .context("mihomo proxy provider has no proxies array")?;
    Ok(Some(
        proxies
            .iter()
...
mod tests {
    ...
    #[test]
    fn parses_proxy_provider_proxy_names() -> Result<()> {
        let value = json!({
            "providers": {
                "check": {
                    "proxies": [
        ...
        assert_eq!(
            proxy_provider_proxy_names_from_value(&value, "check")?,
            Some(vec!["HK 01".into(), "JP 01".into()])
        );
        assert_eq!(
            proxy_provider_proxy_names_from_value(&value, "missing")?,
            None
        );
        Ok(())
    }
